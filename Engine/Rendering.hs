...
import Apecs (Not, cfoldM, cfoldM_, cmapM_, get)
...
import Engine.Constants (ptsFontPath, pxFontPath)
import Engine.DataTypes (ClickState (..), GameTextures, StructureState (Construction, Enabled, Placement))
import Engine.Utils (getRelativeBoxPosition, gget)
import Engine.World (System')
import Foreign.C.Types (CInt)
import Linear (V2 (..), V4 (..))
import SDL (Texture, ($=))
import qualified SDL
import SDL.Font (Color, Font, size, solid)
...
drawUIBoxes renderer tileset =
  cmapM_ $
    \( Button cState hover toggled,
       InterfaceBox bSize@(V2 bw bh),
       Position pos@(V2 x y),
       Sprite coord size@(V2 sw sh) scale
       ) -> do
...
drawBuildingInfo
  ...
  baseCount
  count
  (Building _, EntityName name, ConstructionStorage space, Just request) = do
    if count < baseCount
      then case mFont of
        Just font -> do
...
drawBuildingInfo
  ...
  baseCount
  count
  (Building _, EntityName name, ConstructionStorage space, Nothing) = do
    if count < baseCount
      then case mFont of
        Just font -> do
