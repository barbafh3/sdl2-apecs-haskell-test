...
instance Monoid WhiteFont where mempty = WhiteFont $ Just ""
instance Component WhiteFont where type Storage WhiteFont = Global WhiteFont

newtype BlackFont = BlackFont (Maybe String) deriving Show
instance Semigroup BlackFont where (<>) = mappend
instance Monoid BlackFont where mempty = BlackFont $ Just ""
