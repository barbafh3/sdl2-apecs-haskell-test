...
import Apecs (Component (..), Entity, Global, Map)
...
data HaulTaskN = HaulTaskN [Entity] [Entity] StorageItem Entity Entity deriving (Show)

instance Component HaulTaskN where type Storage HaulTaskN = Map HaulTaskN

data HaulRequest = HaulRequest StorageList [Entity] deriving (Show)
...
data TaskManager = TaskManager [Haul] [Build] deriving (Show)

instance Semigroup TaskManager where (<>) = mappend

instance Monoid TaskManager where mempty = TaskManager [] []

instance Component TaskManager where type Storage TaskManager = Global TaskManager
